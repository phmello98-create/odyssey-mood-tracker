...
export const scheduledBotPost = functions.pubsub
    ...
    .onRun(async (context) => {
        ...
        if (hour >= QUIET_HOURS.START && hour < QUIET_HOURS.END) {
            console.log(`😴 Quiet hours (${QUIET_HOURS.START}h-${QUIET_HOURS.END}h) - skipping`);
            return null;
        }
        ...
        if (todayPostsQuery.size >= RATE_LIMITS.MAX_BOT_POSTS_PER_DAY) {
            console.log(`📊 Daily limit reached (${todayPostsQuery.size}/${RATE_LIMITS.MAX_BOT_POSTS_PER_DAY})`);
            return null;
        }
        ...
        // Gerar conteúdo
        const content = await generateBotContent(botId, { context: 'regular' });
